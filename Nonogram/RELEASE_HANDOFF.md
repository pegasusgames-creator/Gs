# Release Handoff — Nonogram Puzzle
...
`Nonogram/android/app/src/main/AndroidManifest.xml` and
`Nonogram/android/app/src/main/java/com/pegasusgames/nonogram/MainActivity.java`.
...
Play Console → Nonogram Puzzle → **Grow → Main store listing**.
...
Open `Nonogram/metadata/en-US/full_description.txt` and paste the
entire contents.
...
- Tags: pull from `Nonogram/metadata/en-US/keywords.txt`
...
Play Console → Nonogram Puzzle → **Policy → App content**.
...
```
python3 build_release.py Nonogram
```

Output AAB will be at:
```
Nonogram/android/app/build/outputs/bundle/release/app-release.aab
```
...
Play Console → Nonogram Puzzle → **Test and release → Production → Create new release → Upload**.
