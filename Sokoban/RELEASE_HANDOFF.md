# Release Handoff — Sokoban — Box Pushing Puzzle
...
The AAB at
`Sokoban/android/app/build/outputs/bundle/release/app-release.aab`
was built with:
...
2. Apply for / verify your AppLovin app entry at
   ...
   `sokoban_banner`, `sokoban_interstitial`, `sokoban_rewarded`.
3. Paste into
   `Sokoban/android/app/src/main/java/com/pegasusgames/sokoban/MainActivity.java`:
...
4. Re-add the `applovin.sdk.key` meta-data in
   `Sokoban/android/app/src/main/AndroidManifest.xml` with the real key.
...
Sokoban consistent with the rest of the portfolio.
...
- **App name:** `Sokoban — Box Pushing Puzzle`
...
Play Console → Sokoban — Box Pushing Puzzle → **Monetize → Products → In-app products**.
...
Play Console → Sokoban — Box Pushing Puzzle → **Grow → Main store listing**.
...
Open `Sokoban/metadata/en-US/full_description.txt` and paste the
entire contents.

**Graphics:**
- App icon → upload `Sokoban/store/icon_512_playstore.png`
- Feature graphic → upload `Sokoban/store/feature_graphic_1024x500.png`
- Phone screenshots → upload all 7 files in `Sokoban/store/screenshots/phone/`
- 7-inch tablet → upload 2 file(s) in `Sokoban/store/screenshots/tablet_7/`
- 10-inch tablet → upload 2 file(s) in `Sokoban/store/screenshots/tablet_10/`

If Play Console rejects 2 tablet screenshots (Google requires min 4),
open `Sokoban/wrap_tablet_screenshots.py`, uncomment the lines under
`EXTRA_SCREENSHOTS`, re-run, re-upload.

**Categorization:**
- App category: `GAME_PUZZLE`
- Tags: pull from `Sokoban/metadata/en-US/keywords.txt`
...
|---|---|
| Arabic | `Sokoban/metadata/ar/` |
| German (Germany) | `Sokoban/metadata/de-DE/` |
| Spanish (Latin America) | `Sokoban/metadata/es-419/` |
| French (France) | `Sokoban/metadata/fr-FR/` |
| Hindi (India) | `Sokoban/metadata/hi-IN/` |
| Indonesian | `Sokoban/metadata/id/` |
| Italian (Italy) | `Sokoban/metadata/it-IT/` |
| Japanese (Japan) | `Sokoban/metadata/ja-JP/` |
| Portuguese (Brazil) | `Sokoban/metadata/pt-BR/` |
| Turkish (Turkey) | `Sokoban/metadata/tr-TR/` |
| Ukrainian | `Sokoban/metadata/uk/` |
| Chinese (Simplified) | `Sokoban/metadata/zh-CN/` |
...
Play Console → Sokoban — Box Pushing Puzzle → **Policy → App content**.
...
```
python3 build_release.py Sokoban
```
...
Play Console → Sokoban — Box Pushing Puzzle → **Test and release → Production → Create new release → Upload**.
