...
- **Shipped (4):** WS (vc 43 / v2.1.20), Nono (vc 27 / v1.2.19), P2048 (vc 29 / v1.2.20), UB (vc 27 / v1.2.19, live 2026-05-29, 1+ downloads). Retention-complete. 13 locales; phone + tablet 7"/10". All 4 pass `pre_publish_check` clean (2026-06-22). Have shipped several updates past the original launch builds (menu redesign + settings + per-app QA fixes through 2026-06-18). Next release: bump versionCode, rebuild AAB, upload.
- **In ship prep:** PipeConnect (vc 17 / v1.8.1). Full metadata, 13 locales, 4-tier ladder, dedicated keystore. AdMob test-IDs (`ca-app-pub-3940256099942544`) clear at RELEASE_HANDOFF Step 1. The 3 blockers from 2026-05-25 (`weekly_pass` SUBS routing, Restore Purchases UI, interstitial counter-gating) are all RESOLVED — `pre_publish_check` clean as of 2026-06-22 (only the by-construction flow-solvability WARN remains). **2026-06-16: all 122 hand-authored levels were unsolvable — regenerated solvable-by-construction (see fix log); `check_pipeconnect_solvable` gate added.**
- **Also unreleased, near-ready:** Afterimage, Hunch, Overlay — full menu/growth-shim stack, 500-level campaigns, 13 locales; `pre_publish_check` clean except WARN-only items (PGS `LEADERBOARD_ID` placeholder — see `scripts/growth_open_items.md` §B; renders as synthetic "Practice bracket" until a real id is wired). **Sokoban** is the least complete unreleased app: 500 pull-generated solvable levels + metadata exist, but it has NOT yet received the shared menu/growth-shim rollout (game.html ~77 KB vs ~340 KB for its siblings; no MENU/freecoins/Ranks shim) — needs `reinject_all_shims.py` + emulator verification before it matches the family bar.
