...
- **Shipped (4):** WS (vc 44 / v2.1.21), Nono (vc 28 / v1.2.20), P2048 (vc 30 / v1.2.21), UB (vc 28 / v1.2.20, live 2026-05-29, 1+ downloads). Retention-complete. 13 locales; phone + tablet 7"/10". All 4 pass `pre_publish_check` clean (2026-06-22). **2026-06-22 release built:** signed release AABs produced for all 4 (`bundleRelease`), substantive feature-led release notes (redesigned menu / Play Games leaderboards / weekly tournament / daily challenges, not just button polish) across 13 locales. P2048 this round also: retired the duplicate High-Scores screen (leftmost menu tile → Themes), rebuilt Stats/Missions cards, fixed the cross-promo install reward (see fix log); Nono got the same cross-promo fix. **Upload pending** (user uploads via RELEASE_HANDOFF).
- **Finished unreleased (4) — signed release AABs ready to upload:** PipeConnect (vc 18 / v1.8.2, rebuilt 2026-06-23), Afterimage (vc 1), Hunch (vc 1), Overlay (vc 1). Full menu/growth-shim stack, 500-level campaigns, 13 locales, dedicated per-app keystores, feature-led release notes. **Real PGS leaderboard IDs wired 2026-06-22** (Hunch `…CQ`, Afterimage `…Cg`, Overlay `…Cw`; the 3 Play-Console leaderboards are "draft / ready to publish" — publish them there before/with upload). All 4 pass `pre_publish_check` clean (PipeConnect keeps only the by-construction flow-solvability WARN). AdMob test-IDs (`ca-app-pub-3940256099942544`) clear at RELEASE_HANDOFF Step 1. PipeConnect's 3 blockers from 2026-05-25 (SUBS routing / Restore UI / interstitial gating) are all resolved. **2026-06-16: PipeConnect's 122 hand-authored levels were unsolvable — regenerated solvable-by-construction (see fix log); `check_pipeconnect_solvable` gate added.**
- **NOT finished — do not upload:** **Sokoban** (vc 12). 500 pull-generated solvable levels + metadata + a signed AAB exist, but it FAILS `pre_publish_check` hard (16 growth-baseline blockers, unreplaced placeholders, SUBS routing) because it never received the shared menu/growth-shim rollout — its menu is a bespoke dark-default overlay (`buildMenu`→`#sokoMenu`), not the shared `showScreen`/`.menu-icon-row` architecture, so `reinject_all_shims.py` has no anchors to patch. Needs a real menu/growth rewrite + emulator verification before it joins the finished set.
...
- **Cross-promo install reward read the wrong field (2026-06-22).** `PROMO_GAMES` entries declare the target under `packageId:`, but `renderMoreGames`/`claimPromo`/`checkCrossPromoInstalls` read `g.pkg` — so `Android.isAppInstalled(undefined)` always returned false and the +200-per-install reward silently never auto-claimed (Nonogram + Puzzle2048; WaterSort already read `g.packageId`, UnblockPuzzle uses a `pkg:` field). Fixed with a one-line `PROMO_GAMES.forEach(g=>{ if(!g.pkg) g.pkg=g.packageId; })` normaliser after the array. `check_cross_promo_pkg_field.py` gates it (don't flag files that read `g.packageId` anywhere — a stray `g.pkg` can belong to a separate `pkg:`-field promo array like WaterSort's ALL_PROMO).
- **Puzzle2048 redundant High-Scores screen (2026-06-22).** The `scoresScreen` (Best Score / Highest Tile / Login Streak) strictly duplicated the Stats screen; removed it (+ its dead `updateUI` writes and show-handler) and repointed the menu's leftmost icon-row tile from `Best`→`scoresScreen` to `Themes`→`themeScreen`. `check_menu_composition`'s leftmost slot now accepts `{Levels, Best, Themes}`. Also fixed the Daylight miss where `.stat-card`/`.mission-card`/`.mission-bar-bg`/`#themesGrid`/`#statsList`/`#missionsList` used white-on-light surfaces + `justify-content:center` (invisible cards floating mid-screen) — switched to dark-on-light base + `flex-start` + higher-specificity midnight overrides. **Lesson: any screen whose list container uses `flex:1` MUST be `justify-content:flex-start` (center floats few items to mid-screen); any card surface MUST use dark-on-light base + midnight override, never `rgba(255,255,255,*)` (invisible on the light default).**
- **Nonogram curated boards bypassed the uniqueness gate (2026-06-23).** `buildLevels()` uses `PATTERNS_5[i]`/`PATTERNS_10[i]` DIRECTLY for levels 1-70 — `_generateUnique()` only runs in the never-taken fallback branch. A solution-counter found PATTERNS_5 levels **30/45/46/50 each had 2 solutions** (player completes a different picture than stored and still "wins"). Replaced each with a single-cell-edited unique board (no empty-row/col change). New `scripts/verify_nonogram.py` reconstructs all 500 boards exactly as `buildLevels()` does and asserts exactly one solution (line-solver + bounded branch, pure Python, no node, ~21s); `check_nonogram_unique.py` now delegates to it (it used to verify only PREGEN via node and explicitly TRUSTED the curated arrays). **Lesson: any curated/hand-authored puzzle array used directly — bypassing the runtime generation gate — must be offline-verified; never "trust" hand-drawn boards.**
- **PipeConnect 2★/3★ reachability — fillability gate (2026-06-23).** Stars score by coverage (`3★`=all cells filled + no resets, `2★`=all filled, `1★`=connected), so a level that connects but can't be fully filled caps the player at 1★. The current levels ARE fully fillable (the `gen_levels.py` Hamiltonian generator runs `validate()` for full coverage before emitting each level, and reproduces all 500 shipped boards byte-for-byte), but the existing `check_pipeconnect_solvable` only asserts winnability. Refactored `gen_levels.py` to expose `build_levels()`; new `scripts/check_pipeconnect_fillable.py` asserts `game.html` LEVELS == the validated generator output (fast, all sizes, no solving) — any hand-edit voids the fill guarantee and blocks the build. No level data changed (the "~55% unfillable" finding was the pre-2026-06-16 hand-authored set).
- **WaterSort had no save schema version (2026-06-23).** WaterSort (most-installed live app) was the only portfolio game without `saveVersion`+migration, so a future level/content change could leave stale out-of-range indices in `completedLevels`/`levelStars`/`currentLevel` and corrupt existing saves. Added `SAVE_VERSION=1` + `migrateSave(s)` (clamps level-keyed maps/arrays to `levels.length`, clamps `currentLevel`, stamps `saveVersion`, runs once) wired into `State.load()`/`save()`. First-run coin seed was already 200 via `load()`; `reset()` was the lone `coins=0` inconsistency — set to 200 to mirror a fresh install (portfolio standard 200 coins / 5 lives). **Every new app must ship `saveVersion`+`migrateSave` before any content change.**
- **PipeConnect listing accuracy (2026-06-23).** Levels are procedurally generated, so the "hand-tuned / hand-crafted / handmade" wording (13 locales) was false — replaced with truthful descriptors (clever/elegant). Also corrected a factual board-size error: listings claimed boards "up to 10×10" but the generator schedule tops out at **9×9** — fixed 10×10→9×9 everywhere. (Cross-promo self-exclusion was VERIFIED already correct — manifest `<queries>`, `CROSS_PROMO_PACKAGES`, `PROMO_GAMES`, and `ALL_PROMO` all omit own package; the "appears to include self" was the `currentPkg()` heuristic, not a target list.)
