...
- **Puzzle2048 redundant High-Scores screen (2026-06-22).** The `scoresScreen` (Best Score / Highest Tile / Login Streak) strictly duplicated the Stats screen; removed it (+ its dead `updateUI` writes and show-handler) and repointed the menu's leftmost icon-row tile from `Best`→`scoresScreen` to `Themes`→`themeScreen`. `check_menu_composition`'s leftmost slot now accepts `{Levels, Best, Themes}`. Also fixed the Daylight miss where `.stat-card`/`.mission-card`/`.mission-bar-bg`/`#themesGrid`/`#statsList`/`#missionsList` used white-on-light surfaces + `justify-content:center` (invisible cards floating mid-screen) — switched to dark-on-light base + `flex-start` + higher-specificity midnight overrides.
